...
export default function InsightsSection() {
  ...
  return (
    <section
      id="insights"
      ref={ref}
      className="relative py-20 sm:py-28 bg-aethon-gray"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-aethon-text">
            Growth <span className="text-gold-gradient">Intelligence</span>
          </h2>

          <p className="mt-6 text-aethon-text-secondary max-w-2xl mx-auto text-base sm:text-lg">
            Strategic insights and frameworks from the frontlines of growth
            engineering.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          {displayInsights.map((insight, i) => (
            <Link key={insight.id} href={`/blogs/${insight.slug}`}>
              <motion.article
                initial={{ opacity: 0, y: 30 }}
                animate={isInView ? { opacity: 1, y: 0 } : {}}
              ...
              >
                ...
                <div className="relative h-40 sm:h-48 overflow-hidden">
                  {insight.image ? (
                    ...
                    <div
                      className={`h-full w-full bg-gradient-to-br ${insight.gradient}`}
                    />
                  )}
                  ...
                  <div className="absolute bottom-4 left-4 z-10">
                    <Badge
                      className={`${
                        ...
                        'bg-aethon-gold text-white'
                      } text-xs border-0 rounded-full px-3 py-1`}
                    >
                      {insight.category}
                    </Badge>
                  </div>
